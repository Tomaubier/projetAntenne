...
class PlaneWave(Medium):
    ...
    def static_field(self, x, y):
        field = np.zeros((x.shape[0], x.shape[0]), dtype='cfloat')
        for ii in range(x.shape[0]):
            for jj in range(x.shape[0]):
                field[ii, jj] = np.exp(1j*np.dot(self.k, [x[ii, jj], y[ii, jj]]))
        return field
