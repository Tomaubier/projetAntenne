...
class CircularArray:
    def __init__(self, M=6, R=4.32e-2): # 1st order: M=3 / 3rd: M=6
...
class DifferentialArray(CircularArray, Medium):
    ...
    def filter_vector(self, f):
        ...
        theta_c = np.array([0, np.pi/2, 2*np.pi/3, np.pi]) + self.dphi*self.steering_mic # 1st order: [0, np.pi] / 3rd: [0, np.pi/2, 2*np.pi/3, np.pi]
